...
雖然我們最初在 CSS 變量中包含斷點（例如 `--breakpoint-md`），**但是 media queries 中不支持這些斷點**，但它們仍然可以在 media queries 中使用這些規則集。這些斷點變量保留在已編譯的 CSS 中以便向後兼容，因為它們可以被JavaScript 使用。 [在規範中了解更多](https://www.w3.org/TR/css-variables-1/#using-variables)
