...
### 改變動畫 {#change-animation}

`$modal-fade-transform` 變數在互動視窗執行淡入動畫前會先確定 `.modal-dialog` 的變換狀態，當互動視窗的淡入動畫結束， `$modal-show-transform` 變數會確定 `.modal-dialog` 的變換。
...
動態視窗有三個可選的大小，可以通過 Class 放在 `.modal-dialog` 上。 這些尺寸會在某些中斷點調整，以避免在較窄的 viewport 上出現水平捲軸。
