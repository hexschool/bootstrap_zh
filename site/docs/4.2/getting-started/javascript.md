...
如果您使用 bundler (Webpack, Rollup...)，可以使用 UMD 就緒的 `/js/dist/*.js` 檔案。
...
目前，為了查詢DOM元素，出於性能原因，我們使用原生方法 `querySelector` 和 `querySelectorAll`，因此必須使用 [valid selectors](https://www.w3.org/TR/CSS21/syndata.html#value-def-identifier)。
