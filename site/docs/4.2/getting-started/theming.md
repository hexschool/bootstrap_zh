...
利用我們的 Sass 原始檔來調整變數、Sass maps、mixins 還有其它等等。在我們的建構中，我們將 Sass 引入精度提高到 6 (預設為 5) 以防止瀏覽器引入問題。
...
您可以在 `scss/_variables.scss` 中找到 Bootstrap 變數的完整列表。
...
移除色彩從 `$theme-colors`，或其他的 map 可用 `map-remove`。請注意，您必須在我們的必要(Required)和選用(Optional)之間插入它：
...
你可以從 `scss/_variables.scss` 檔案中的全域選項中找到這些自定義變數。
...
| `$enable-prefers-reduced-motion-media-query`       | `true` (default) or `false`        | 啟用 [`prefers-reduced-motion` media query]({{ site.baseurl }}/docs/{{ site.docs_version }}/getting-started/accessibility/#reduced-motion)，根據用戶的瀏覽器 / 操作系統選項抑制某些動畫 / 轉換。|
...
| `$enable-validation-icons`  | `true` (default) or `false`        | 在文本輸入中啟用 `background-image` 圖示，為自定義表單啟用驗證狀態 |
...
Bootstrap 4 中的所有顏色都可用作 Sass 變數和 `scss/_variables.scss` 文件中的 Sass 對應。這將在下一個次要版本增加額外的陰影，我們已經包含樣式如[grayscale palette](#grays)。）
...
我們使用所有色彩的子集來創建一個小的色表，用來產生色彩主題，也同時能透過 Bootstraps 的 `scss/_variables.scss` 使用 Sass 變數、Sass map。
...
`scss/_variables.scss` 中設置了一組廣泛的灰色變數和一個 Sass maps，整個專案將有一致的灰色陰影。請注意，這些是 “冷灰色”，傾向於微妙的藍色調，而不是中性灰色。
...

在 `scss/_variables.scss` 中，你會發現我們的顏色變數和 Sass maps。以下是 `$colors` Sass maps 的範例：
...
Bootstrap 4 編譯時包含約 24 個 [CSS 自定義屬性 (變數)](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_variables)，這提供了像是主題色彩、中斷點，以及原型設計、沙盒、瀏覽器檢查中的主要的字體堆疊。
...
### 中斷點變數 {#breakpoint-variables}

雖然我們最初在 CSS 變量中包含斷點（例如 `--breakpoint-md`），**但是 media queries 中不支持這些斷點**，但它們仍然可以在 media queries 中使用這些規則集。這些斷點變量保留在已編譯的 CSS 中以便向後兼容，因為它們可以被 JavaScript 使用。 [在規範中了解更多](https://www.w3.org/TR/css-variables-1/#using-variables)
